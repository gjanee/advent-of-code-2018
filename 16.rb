...
# while before you reach your next destination, and with a bit of
# trial and error, you manage to pull up a programming manual on the
...
# with that number.  So, if the opcode addi adds register A and
# value B, storing the result in register C, and the instruction
...
# doesn't seem to indicate the number.  However, you can monitor the
# CPU to see the contents of the registers before and after
# instructions are executed to try to work them out.  Each opcode has
# a number from 0 through 15, but the manual doesn't say which is
# which.  For example, suppose you capture the following sample:
#
# Before: [3, 2, 1, 1]
# 9 2 1 2
# After:  [3, 2, 2, 1]
...
# 3, register 1 has value 2, and registers 2 and 3 have value 1.
...
Functions = [
  lambda {|a,b,c,r| r[c] = r[a]+r[b] },           # addr
  lambda {|a,b,c,r| r[c] = r[a]+b },              # addi
  lambda {|a,b,c,r| r[c] = r[a]*r[b] },           # mulr
  lambda {|a,b,c,r| r[c] = r[a]*b },              # muli
  lambda {|a,b,c,r| r[c] = r[a]&r[b] },           # banr
  lambda {|a,b,c,r| r[c] = r[a]&b },              # bani
  lambda {|a,b,c,r| r[c] = r[a]|r[b] },           # borr
  lambda {|a,b,c,r| r[c] = r[a]|b },              # bori
  lambda {|a,b,c,r| r[c] = r[a] },                # setr
  lambda {|a,b,c,r| r[c] = a },                   # seti
  lambda {|a,b,c,r| r[c] = a > r[b] ? 1 : 0 },    # gtir
  lambda {|a,b,c,r| r[c] = r[a] > b ? 1 : 0 },    # gtri
  lambda {|a,b,c,r| r[c] = r[a] > r[b] ? 1 : 0 }, # gtrr
  lambda {|a,b,c,r| r[c] = a == r[b] ? 1 : 0 },   # eqir
  lambda {|a,b,c,r| r[c] = r[a] == b ? 1 : 0 },   # eqri
  lambda {|a,b,c,r| r[c] = r[a] == r[b] ? 1 : 0 } # eqrr
]

Input = open("16.in").readlines

# samples: [[opcode, [index into Functions of possible match, ...]], ...]
samples = Input.slice(0, 2+Input.rindex {|l| l.start_with?("After") })
  .each_slice(4)
  .map {|l1,l2,l3,_|
    pre         = l1.scan(/\d+/).map(&:to_i)
    op, a, b, c = l2.scan(/\d+/).map(&:to_i)
    post        = l3.scan(/\d+/).map(&:to_i)
    [op,
      Functions.map.with_index {|f,i|
        r = pre.dup
        # It's conceivable that the magnitude of an operand could
        # force its interpretation as value as opposed to register.
        # But there are no operands outside 0..3 in the input, so the
        # following call can be made without any error checking.
        f.call(a, b, c, r)
        [i, r == post]
      }.select {|i,m| m }.map(&:first)
    ]
  }

puts samples.count {|op,l| l.length >= 3 }
...
# --------------------
#
# The puzzle supports simple, direct deduction: at each step there is
# at least one unambiguous opcode.  After an opcode has been
# determined, all references to the opcode and to the function it
# identifies are removed, which yields a new unambiguous opcode.

opcodes = [nil]*Functions.length # opcode => index into Functions
while samples.length > 0
  s = samples.find {|op,l| l.length == 1 }
  s_op = s[0]
  s_fid = s[1][0]
  opcodes[s_op] = s_fid
  samples.reject! {|op,l| op == s_op }
  samples.each do |op,l|
    l.reject! {|fid| fid == s_fid }
  end
end

r = [0, 0, 0, 0]
Input[4+Input.rindex {|l| l.start_with?("After") }..-1].each do |l|
  op, a, b, c = l.scan(/\d+/).map(&:to_i)
  Functions[opcodes[op]].call(a, b, c, r)
end
puts r[0]

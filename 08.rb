...
Input = open("08.in").read.split.map {|v| v.to_i }
...
class Node
  ...
  def metadata_sum
    @children.map {|c| c.metadata_sum }.reduce(0, :+) + @metadata.reduce(:+)
  end
...
def load_node
  nc = next_value
  nm = next_value
  c = nc.times.map { load_node }
  m = nm.times.map { next_value }
  Node.new(c, m)
end

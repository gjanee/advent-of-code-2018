...
# formed by extremely long polymers (one of which is available as your
# puzzle input).
#
...
Input = open("05.in").read.chomp

def react(s)
  a = []
  s.each_char do |c|
    if a.length > 1 && c == a[-1].swapcase
      a.pop
    else
      a.push(c)
    end
...
puts ("a".."z").map {|c| react(Input.gsub(/#{c}|#{c.upcase}/, "")) }.min

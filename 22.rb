...
# outside; the only light you can see is coming from a small cottage
...
#
# After talking for a while, he asks a favor of you.  His friend
# hasn't come back in a few hours, and he's not sure where he is.
# Scanning the region briefly, you discover one life signal in a cave
# system nearby; his friend must have taken shelter there.  The man
# asks if you can go there to retrieve his friend.
#
...
#
# The scan (your puzzle input) is not very detailed: it only reveals
# the depth of the cave system and the coordinates of the target.
# However, it does not reveal the type of each region.  The mouth of
# the cave is at 0,0.
#
...
# geologic index.  The geologic index can be determined using the
# first rule that applies from the list below:
#
# - The region at 0,0 (the mouth of the cave) has a geologic index
#   of 0.
...
# is 114.
#
# What is the total risk level for the smallest rectangle that
# includes 0,0 and the target's coordinates?
#
# --------------------
#
# It will be convenient in part 2 to generate region types on demand,
# so as to be able to extend past the target if necessary.  The hash
# tables below are indexed by x,y coordinate and serve as caches.

Depth = 4848
Target_x = 15
Target_y = 700

geologic_index = Hash.new {|h,k|
  x,y = k
  if (x == 0 && y == 0) || (x == Target_x && y == Target_y)
    ...
    v = $erosion_level[[x-1,y]]*$erosion_level[[x,y-1]]
  end
  h[k] = v
}

$erosion_level = Hash.new {|h,k|
...
# must reach the target coordinates as quickly as possible.  The
...
# region.  Moving to an adjacent region takes one minute.  (For
...
# before you can find him in the dark.  The target is always in a
# rocky region, so if you arrive there with climbing gear equipped,
...
# --------------------
#
# Dijkstra's algorithm.  For each region we create two nodes, one for
# each tool that is compatible with the region's type.  In other
# words, a node represents the joint state of being in a region and
# holding a given tool.  An edge of weight 7 joins a region's two
# nodes, representing the act of changing tools.  The edges to
# neighboring nodes, all of weight 1, occur only between nodes whose
# tools match.  Nodes are created on demand, as encountered; edges are
# not explicitly stored, but computed.  But first, an unfortunate
# omission from the Ruby standard library...
...
R, W, N = 0, 1, 2 # region types: rocky, wet, narrow
T, C, Z = 0, 1, 2 # tools: torch, climbing gear, neither (zilch)
...
class Node
  ...
  def Node.lookup(x,y)
    @@all.fetch([x,y]) {
      type = region_type(x,y)
      @@all[[x,y]] = Allowable_tools[type].map {|t| new(x, y, type, t) }
    }
  end

  def initialize(x, y, type, tool)
    @x, @y, @type, @tool = x, y, type, tool
    @distance = nil
    @visited = false
  end

  def neighbors
    l = @@all[[@x,@y]].reject {|r| r == self }
    [-1, 1].each do |d|
      if @x+d >= 0
        Node.lookup(@x+d,@y).each do |r|
          l << r if r.tool == @tool
        end
      end
      if @y+d >= 0
        Node.lookup(@x,@y+d).each do |r|
          l << r if r.tool == @tool
        end
      end
    end
...
r = Node.lookup(0,0).find {|r| r.tool == T }
r.distance = 0
r.visited = true
unvisited = PriorityQueue.new
while true
  r.neighbors.reject {|n| n.visited }.each do |n|
    d = n.tool == r.tool ? 1 : 7
    n.distance =
      n.distance.nil? ? r.distance+d : [r.distance+d, n.distance].min
    # A node may see its distance decrease some time after having been
    # inserted into `unvisited`.  To avoid rebalancing the priority
    # queue, we simply add a new entry with every distance change, and
    # when removing nodes ignore those already visited.
    unvisited << QueueEntry.new(n)
  end
  r.visited = true
  break if r.x == Target_x && r.y == Target_y && r.tool == T
  while r.visited
    r = unvisited.pop.node
  end
end
puts r.distance

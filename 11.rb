...
# power.  The square must be entirely within the 300x300 grid.
# Identify this square using the X,Y coordinate of its top-left fuel
# cell.  For example:
#
...
SN = 2694 # grid serial number
S = 300 # grid size

# it will be helpful in part 2 to have margins on the left and top
G = (0..S).map {|x|
  (0..S).map {|y|
    x > 0 && y > 0 ? (((x+10)*y+SN)*(x+10))%1000/100-5 : 0
  }
}

puts (1..S-2).map {|x|
  (1..S-2).map {|y|
    [x, y, [0,1,2].product([0,1,2]).map {|dx,dy| G[x+dx][y+dy] }.reduce(:+)]
  }
}.flatten(1).max {|a,b| a[2] <=> b[2] }.slice(0, 2).join(",")

# --- Part Two ---
#
# You discover a dial on the side of the device; it seems to let you
# select a square of any size, not just 3x3.  Sizes from 1x1 to
# 300x300 are supported.
#
...
# a third parameter after the top-left coordinate: a 9x9 square with a
...
# --------------------
#
# The trick to doing this efficiently is to use a summed-area table
# <https://en.wikipedia.org/wiki/Summed-area_table>, which brings the
# time down to O(n^3).

T = Array.new(S+1) { Array.new(S+1) { 0 } }

(1..S).each do |x|
  (1..S).each do |y|
    T[x][y] = G[x][y] + T[x][y-1] + T[x-1][y] - T[x-1][y-1]
  end
end

puts (1..S).map {|s| # side length
  (s..S).map {|x|
    (s..S).map {|y|
      # [x, y of top left corner, side length, sum]
      [x-s+1, y-s+1, s, T[x][y] - T[x-s][y] - T[x][y-s] + T[x-s][y-s]]
    }
  }
}.flatten(2).max {|a,b| a[3] <=> b[3] }.slice(0, 3).join(",")

...
Input = open("02.in").readlines.map {|l| l.chomp }

def count(n)
  Input.count {|id|
    h = Hash.new(0)
    id.each_char {|c| h[c] += 1 }
    h.value?(n)
  }
end
...
# --------------------
#
# The IDs are all the same length (the puzzle wouldn't make much sense
# if they weren't).

Input.combination(2).each do |id_a,id_b|
  common = id_a.chars.zip(id_b.chars)
    .select {|a,b| a == b }
    .map(&:first)
    .join
  puts common if common.length == id_a.length-1
end

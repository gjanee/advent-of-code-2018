...
# forward 1 + 3 = 4 times, while the second Elf moves forward
# 1 + 7 = 8 times.  If they run out of recipes, they loop back around
# to the beginning.  After the first round, both Elves happen to loop
# around until they land on the same recipe that they had in the
...
def play_until(&block)
  # plays until the block returns true
  ...
  while !yield(sb)
    ...
    sb << v%10
    e1 = (e1+sb[e1]+1)%sb.length
    e2 = (e2+sb[e2]+1)%sb.length
  end
  sb
end

puts play_until {|sb| sb.length >= Input+10 }.slice(Input, 10).join
...
# --------------------
#
# Efficiency is critical here.  The block below is executed more than
# 20 million times, so it's necessary to avoid any object creation
# within the block.  The goal pattern is simple enough in this case
# that a more sophisticated matching algorithm such as
# Knuth-Morris-Pratt is not necessary.

pattern = Input.to_s.chars.map(&:to_i)

puts play_until {|sb|
  next(false) if sb.length < pattern.length
  i = 0
  j = sb.length-pattern.length
  match = true

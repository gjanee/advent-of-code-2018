...
# they count as awake on the minute they wake up.  For example,
...
class Guard
  ...
  def total_nap_time
    @naps.map {|f,t| t-f }.reduce(0, :+)
  end

  def max_naps
    (0..59)
      .map {|m| {minute: m,
        value: @naps.select {|f,t| f <= m && m < t }.count} }
      .max {|x,y| x[:value] <=> y[:value] }
  end
...
open("04.in").readlines.sort.each do |l|
  # N.B.: in the puzzle input the guard always wakes up before the
  # midnight hour is up.
  if l =~ /Guard #(\d+)/
    g = Guards[Regexp.last_match[1].to_i]
  elsif l =~ /(\d\d)\] falls asleep/
    f = Regexp.last_match[1].to_i
  elsif l =~ /(\d\d)\] wakes up/
    g.nap!(f, Regexp.last_match[1].to_i)
  end
end

g = Guards.values.max {|x,y| x.total_nap_time <=> y.total_nap_time }
puts g.id*g.max_naps[:minute]
...
g = Guards.values.max {|x,y| x.max_naps[:value] <=> y.max_naps[:value] }
puts g.id*g.max_naps[:minute]
